...
if __name__=='__main__':
    ...
    start_text_feature,max_length_start = generator.get_clip_text_features(start_prompt)
    end_text_feature,max_length_end = generator.get_clip_text_features(end_prompt)
    max_length = max(max_length_start,max_length_end)

    # generate 20 step interpolation
    text_feature_list = []
    for frame in range(20):
        text_features = torch.lerp(start_text_feature.detach().cpu(), end_text_feature.detach().cpu(), frame/20)
        text_feature_list.append(text_features.to(device))

    latents_list = []
    if args.use_negative_prompt:
        f = open('negative_prompt.txt'); negative_prompt = f.read()
    else: negative_prompt=None
    for text_embedding in text_feature_list:
        latents = generator.generate_conditioned_latents(prompt=None,text_embeddings=text_embedding,max_length=max_length,negative_prompt=negative_prompt)
    ...
    for frame,latent_frame in enumerate(latents_list):
        image = generator.decode_latents(latent_frame)
